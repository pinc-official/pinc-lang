...
and transform_pipe_expression env left right =
  let call =
    match right.expression_desc with
    | FunctionCall { function_definition; arguments } ->
        FunctionCall { function_definition; arguments = left :: arguments }
    | _ -> FunctionCall { function_definition = right; arguments = [ left ] }
  in
  (env, call)
...
and transform_binary_expression env left op right =
  ...
  let env, right = transform_expression env right in
  match op with
  | Operators.Binary.PIPE -> transform_pipe_expression env left right
  | _ ->
