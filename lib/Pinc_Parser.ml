...
type t = {mutable lexer: Lexer.t; mutable token: Token.t; next: Token.t Queue.t}
...
let rec next t =
  ...
  match token.typ with Token.COMMENT -> next t | _ -> t.token <- token
...
let expect token t =
  ...
  if test then next t
    ...
    Diagnostics.report ~start_pos:t.token.start_pos ~end_pos:t.token.end_pos
      (Diagnostics.Message
         ( "Expected: `" ^ Token.to_string token ^ "`, got `"
...
module Helpers = struct
  ...
  let separated_list ~sep ~fn t =
    let rec loop acc =
      ...
      match fn t with
      | Some r ->
          if has_sep || acc = [] then loop (r :: acc)
          else
            Diagnostics.report ~start_pos:t.token.start_pos
  ...
  let list ~fn t =
    let rec loop acc =
      match fn t with Some r -> loop (r :: acc) | None -> List.rev acc
    in
    loop []
end
...
module Rules = struct
  ...
  and parse_template_node t =
    match t.token.typ with
    | Token.STRING s ->
        next t ; Some (Ast.TextTemplateNode s)
    | Token.LEFT_BRACE ->
        next t ;
        let expression =
          match parse_expression t with
          | Some e ->
              Ast.ExpressionTemplateNode e
          | None ->
  ...
  and parse_statement t =
    ...
    | Token.KEYWORD_LET -> (
        ...
        | true, true, Some expression ->
            Some
              (Ast.OptionalMutableLetStatement
                 (Lowercase_Id identifier, expression) )
        ...
        let expression =
          ...
          | None ->
              Diagnostics.report ~start_pos:t.token.start_pos
                ~end_pos:t.token.end_pos
                (Diagnostics.Message
                   "Expected expression as right hand side of mutation \
                    statement" )
        in
        let _ = optional Token.SEMICOLON t in
  ...
  and parse_binary_operator t =
    match t.token.typ with
    | Token.LOGICAL_AND ->
        Some Ast.Operators.Binary.(make AND)
    | Token.LOGICAL_OR ->
        Some Ast.Operators.Binary.(make OR)
    | Token.EQUAL_EQUAL ->
        Some Ast.Operators.Binary.(make EQUAL)
    | Token.NOT_EQUAL ->
        Some Ast.Operators.Binary.(make NOT_EQUAL)
    | Token.GREATER ->
        Some Ast.Operators.Binary.(make GREATER)
    | Token.GREATER_EQUAL ->
        Some Ast.Operators.Binary.(make GREATER_EQUAL)
    | Token.LESS ->
        Some Ast.Operators.Binary.(make LESS)
    | Token.LESS_EQUAL ->
        Some Ast.Operators.Binary.(make LESS_EQUAL)
    | Token.PLUSPLUS ->
        Some Ast.Operators.Binary.(make CONCAT)
    | Token.PLUS ->
        Some Ast.Operators.Binary.(make PLUS)
    | Token.MINUS ->
        Some Ast.Operators.Binary.(make MINUS)
    | Token.STAR ->
        Some Ast.Operators.Binary.(make TIMES)
    | Token.SLASH ->
        Some Ast.Operators.Binary.(make DIV)
    | Token.STAR_STAR ->
        Some Ast.Operators.Binary.(make POW)
    | Token.PERCENT ->
        Some Ast.Operators.Binary.(make MODULO)
    | Token.DOT ->
        Some Ast.Operators.Binary.(make DOT_ACCESS)
    | Token.ARROW_LEFT ->
        Some Ast.Operators.Binary.(make ARRAY_ADD)
    | Token.ATAT ->
        Some Ast.Operators.Binary.(make MERGE)
    | Token.LEFT_BRACK ->
        Some Ast.Operators.Binary.(make BRACKET_ACCESS)
    | Token.LEFT_PAREN ->
        Some Ast.Operators.Binary.(make FUNCTION_CALL)
    | Token.DOTDOT ->
        Some Ast.Operators.Binary.(make RANGE)
    | Token.DOTDOTDOT ->
        Some Ast.Operators.Binary.(make INCLUSIVE_RANGE)
    | Token.PIPE ->
        Some Ast.Operators.Binary.(make PIPE)
    | _ ->
        None
  ...
  and parse_expression ?(prio = -999) t =
    let rec loop ~prio ~left t =
      ...
      | None ->
          left
      | Some {precedence; _} when precedence < prio ->
          left
      ...
      | Some {typ= operator; precedence; assoc; closing_token} -> (
          next t ;
          let new_prio =
            match assoc with Left -> precedence + 1 | Right -> precedence
          in
