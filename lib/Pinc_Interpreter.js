...
function eval_internal_or_external_tag(state, value, tag_info) {
  ...
  if (value !== undefined) {
    ...
    value_bag = Caml_option.valFromOption(value);
  ...
  if (typeof tag === "string") {
    ...
    return call_tag_listener(state, value, tag_info);
  }
}

function call_tag_listener(state, value, t) {
  var attributes = t.attributes;
  var required = t.required;
  var path = t.path;
  var tag = t.tag;
  var listener = Hashtbl.find_opt(state.tag_listeners, tag);
  var v;
  if (typeof tag === "string") {
    if (tag === "Boolean") {
      v = listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Boolean") ? Curry._3(listener.VAL, required, attributes, path) : Result.ok(/* Null */0);
    } else if (tag === "Int") {
      v = listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Int") ? Curry._3(listener.VAL, required, attributes, path) : Result.ok(/* Null */0);
    } else if (tag === "Float") {
      v = listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Float") ? Curry._3(listener.VAL, required, attributes, path) : Result.ok(/* Null */0);
    } else if (tag === "Record") {
      if (listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Record")) {
        var children = Curry._2(StringMap.filter_map, (function (_key, value) {
                return Pinc_Typer.Expect.maybe(Pinc_Typer.Expect.tag_info, value);
              }), Pinc_Typer.Expect.required((function (param) {
                    return Pinc_Typer.Expect.attribute("of", Pinc_Typer.Expect.record, param);
                  }), attributes));
        v = Curry._4(listener.VAL, required, attributes, children, path);
      } else {
        v = Result.ok(/* Null */0);
      }
    } else if (tag === "Slot") {
      v = listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Slot") ? Curry._3(listener.VAL, required, attributes, t.key) : Result.ok(/* Null */0);
    } else if (tag === "Array") {
      if (listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Array")) {
        var child = Pinc_Typer.Expect.required((function (param) {
                return Pinc_Typer.Expect.attribute("of", Pinc_Typer.Expect.tag_info, param);
              }), attributes);
        v = Curry._4(listener.VAL, required, attributes, child, path);
      } else {
        v = Result.ok(/* Null */0);
      }
    } else {
      v = listener !== undefined && !(typeof listener === "string" || listener.NAME !== "String") ? Curry._3(listener.VAL, required, attributes, path) : Result.ok(/* Null */0);
    }
  } else {
    v = listener !== undefined && !(typeof listener === "string" || listener.NAME !== "Custom") ? Curry._5(listener.VAL, tag.VAL, required, attributes, value, path) : Result.ok(/* Null */0);
  }
  if (v.TAG === /* Ok */0) {
