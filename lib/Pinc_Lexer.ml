...
let current_mode t =
  ...
  | Template :: _ ->
      Template
  | TemplateAttributes :: _ ->
      TemplateAttributes
  | ComponentAttributes :: _ ->
      ComponentAttributes
...
let popMode mode t =
  match t.mode with
  | current_mode :: tl when current_mode = mode ->
      t.mode <- tl
  | _ ->
      ()
...
let rec skip_whitespace t =
  if is_whitespace t.current then (next t ; skip_whitespace t)
...
let scan_ident t =
  ...
  let rec loop t =
    ...
    | `Chr ('0' .. '9' as c)
    | `Chr ('_' as c) ->
        next t ; Buffer.add_char buf c ; loop t
    | _ ->
        ()
  in
  let () = loop t in
...
let scan_string ~start_pos t =
  ...
  let rec loop t =
    ...
    | `Chr '\\' -> (
      match peek t with
      | `Chr ' ' ->
          next_n ~n:2 t ; Buffer.add_char buf '\\' ; loop t
      | `Chr '"' ->
          next_n ~n:2 t ; Buffer.add_char buf '"' ; loop t
      | `Chr '\'' ->
          next_n ~n:2 t ; Buffer.add_char buf '\'' ; loop t
      | `Chr 'b' ->
          next_n ~n:2 t ; Buffer.add_char buf '\b' ; loop t
      | `Chr 'f' ->
          next_n ~n:2 t ; Buffer.add_char buf '\012' ; loop t
      | `Chr 'n' ->
          next_n ~n:2 t ; Buffer.add_char buf '\n' ; loop t
      | `Chr 'r' ->
          next_n ~n:2 t ; Buffer.add_char buf '\r' ; loop t
      | `Chr 't' ->
          next_n ~n:2 t ; Buffer.add_char buf '\t' ; loop t
      | `Chr '\\' ->
          next_n ~n:2 t ; Buffer.add_char buf '\\' ; loop t
      | `Chr _ ->
          Diagnostics.report ~start_pos:(make_position t)
...
let scan_tag t =
  ...
  | _ ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
        (Diagnostics.Message
           ( "Invalid Tag! Tags have to start with an uppercase character. Did \
              you mean to write #"
...
let get_html_tag_ident t =
  ...
  let rec loop buf t =
    ...
    | `Chr ('a' .. 'z' as c) | `Chr ('0' .. '9' as c) | `Chr ('-' as c) ->
        next t ; Buffer.add_char buf c ; loop buf t
    | _ ->
        Buffer.contents buf
  in
  let buf = Buffer.create 32 in
...
let scan_component_open_tag t =
  ...
  match t.current with
  | `Chr 'A' .. 'Z' ->
      Token.COMPONENT_OPEN_TAG (get_uppercase_ident t)
  | `Chr c ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
        (Diagnostics.Message
           ( "Invalid Component tag! Component tags have to start with an \
              uppercase letter. Instead saw: " ^ String.make 1 c ) )
  | `EOF ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
...
let scan_open_tag t =
  ...
  | `Chr c ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
        (Diagnostics.Message
           ( "Invalid Template tag! Template tags have to start with a \
              lowercase letter. Instead saw: " ^ String.make 1 c ) )
  | `EOF ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
        Diagnostics.NonTerminatedTemplate
...
let scan_component_close_tag t =
  ...
  match t.current with
  | `Chr '>' ->
      next t ; Token.COMPONENT_CLOSE_TAG close_tag
  | _ ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
...
let scan_close_tag t =
  ...
  let close_tag =
    match t.current with
    | `Chr 'a' .. 'z' ->
        Token.HTML_CLOSE_TAG (get_html_tag_ident t)
    | `Chr c ->
        Diagnostics.report ~start_pos ~end_pos:(make_position t)
          (Diagnostics.Message
             ( "Invalid Template tag! Template tags have to start with a \
  ...
  let rec loop buf t =
    ...
    | `Chr '<' -> (
      ...
      | `Chr 'a' .. 'z' | `Chr 'A' .. 'Z' ->
          Buffer.contents buf
      | _ ->
          next t ; Buffer.add_char buf '<' ; loop buf t )
    | `Chr c ->
        next t ; Buffer.add_char buf c ; loop buf t
  in
  let buf = Buffer.create 32 in
...
let scan_number t =
  ...
  let rec scan_digits t =
    match t.current with
    | `Chr ('0' .. '9' as c) ->
        Buffer.add_char result c ; next t ; scan_digits t
    | `Chr '_' ->
        next t ; scan_digits t
    | _ ->
        ()
  in
  scan_digits t ;
  let is_float =
    ...
    | `Chr '.' -> (
      match peek t with
      | `Chr '.' ->
          false (* Two Dots in a row mean that this is a range operator *)
      | _ ->
          Buffer.add_char result '.' ; next t ; scan_digits t ; true )
    | _ ->
        false
  in
  (* exponent part *)
  ...
  if is_float then Token.FLOAT (float_of_string result)
...
let rec scan_template_token ~start_pos t =
  match t.current with
  | `Chr '{' ->
      setMode Normal t ; next t ; Token.LEFT_BRACE
  | `Chr '}' ->
      popMode Normal t ; next t ; Token.RIGHT_BRACE
  | `Chr '<' -> (
    match peek t with
    | `Chr '>' ->
        next_n ~n:2 t ; setMode Template t ; Token.HTML_OPEN_FRAGMENT
    | `Chr 'a' .. 'z' ->
        setMode Template t ;
    ...
    | `Chr '/' -> (
        ...
        | `Chr 'a' .. 'z' ->
            scan_close_tag t
        | `Chr 'A' .. 'Z' ->
            scan_component_close_tag t
        | `Chr '>' ->
            next_n ~n:3 t ; Token.HTML_CLOSE_FRAGMENT
        | `Chr c ->
            Diagnostics.report ~start_pos ~end_pos:(make_position t)
              (Diagnostics.Message
                 ( "Invalid Template tag! Template tags have to start with an \
                    uppercase or lowercase letter. Instead saw: "
                 ^ String.make 1 c ) )
        | `EOF ->
            Diagnostics.report ~start_pos ~end_pos:(make_position t)
              Diagnostics.NonTerminatedTemplate )
    | _ ->
        scan_template_text t )
  | `Chr '/' -> (
    match peek t with
    | `Chr '>' ->
        popMode Template t ;
    ...
    | `EOF ->
        Diagnostics.report ~start_pos ~end_pos:(make_position t)
          Diagnostics.NonTerminatedTemplate )
  | `Chr '>' ->
      next t ; Token.HTML_OR_COMPONENT_TAG_END
  | `EOF ->
      Diagnostics.report ~start_pos ~end_pos:(make_position t)
        Diagnostics.NonTerminatedTemplate
  | _ ->
...
and scan_string_token ~start_pos t =
  match t.current with
  | `Chr '"' ->
      popMode String t ; next t ; Token.DOUBLE_QUOTE
  | `Chr '{' when peek t = `Chr '|' ->
      setMode Normal t ; next_n ~n:2 t ; Token.LEFT_PIPE_BRACE
  | `Chr '|' when peek t = `Chr '}' ->
      popMode Normal t ; next_n ~n:2 t ; Token.RIGHT_PIPE_BRACE
  | _ ->
      scan_string ~start_pos t
...
and scan_component_attributes_token ~start_pos t =
  ...
  | `Chr '_' | `Chr 'a' .. 'z' ->
      scan_ident t
  | `Chr '"' ->
      next t ; setMode String t ; Token.DOUBLE_QUOTE
  | `Chr '=' ->
      next t ; Token.EQUAL
  | `Chr '{' ->
      setMode Normal t ; next t ; Token.LEFT_BRACE
...
and scan_normal_token ~start_pos t =
  ...
  | `Chr '"' ->
      next t ; setMode String t ; Token.DOUBLE_QUOTE
  | `Chr '(' ->
      next t ; Token.LEFT_PAREN
  | `Chr ')' ->
      next t ; Token.RIGHT_PAREN
  | `Chr '[' ->
      next t ; Token.LEFT_BRACK
  | `Chr ']' ->
      next t ; Token.RIGHT_BRACK
  | `Chr '{' ->
      setMode Normal t ; next t ; Token.LEFT_BRACE
  | `Chr '}' ->
      popMode Normal t ; next t ; Token.RIGHT_BRACE
  | `Chr ':' -> (
    match peek t with
    | `Chr ':' ->
        next_n ~n:2 t ; Token.DOUBLE_COLON
    | `Chr '=' ->
        next_n ~n:2 t ; Token.COLON_EQUAL
    | _ ->
        next t ; Token.COLON )
  | `Chr ',' ->
      next t ; Token.COMMA
  | `Chr ';' ->
      next t ; Token.SEMICOLON
  | `Chr '-' -> (
      if is_whitespace (peek t) then (next t ; Token.MINUS)
      else
        match peek t with
        | `Chr '>' ->
            next_n ~n:2 t ; Token.ARROW
        | _ ->
            next t ; Token.UNARY_MINUS )
  | `Chr '+' -> (
    match peek t with
    | `Chr '+' ->
        next_n ~n:2 t ; Token.PLUSPLUS
    | _ ->
        next t ; Token.PLUS )
  | `Chr '%' ->
      next t ; Token.PERCENT
  | `Chr '?' ->
      next t ; Token.QUESTIONMARK
  | `Chr '/' -> (
    match peek t with
    | `Chr '/' ->
        skip_comment t ; Token.COMMENT
    | _ ->
        next t ; Token.SLASH )
  | `Chr '.' -> (
    match peek t with
    ...
    | `Chr '.' -> (
      match peek ~n:2 t with
      | `Chr '.' ->
          next_n ~n:3 t ; Token.DOTDOTDOT
      | _ ->
          next_n ~n:2 t ; Token.DOTDOT )
    | _ ->
        next t ; Token.DOT )
  | `Chr '@' -> (
    match peek t with
    | `Chr '@' ->
        next_n ~n:2 t ; Token.ATAT
    | _ ->
        Diagnostics.report ~start_pos ~end_pos:(make_position t)
  ...
  | `Chr '&' -> (
    match peek t with
    | `Chr '&' ->
        next_n ~n:2 t ; Token.LOGICAL_AND
    | _ ->
        Diagnostics.report ~start_pos ~end_pos:(make_position t)
          (Diagnostics.UnknownCharacter '&') )
  | `Chr '|' -> (
    match peek t with
    | `Chr '|' ->
        next_n ~n:2 t ; Token.LOGICAL_OR
    | `Chr '}' ->
        popMode Normal t ; next_n ~n:2 t ; Token.RIGHT_PIPE_BRACE
    | `Chr '>' ->
        next_n ~n:2 t ; Token.PIPE
    | _ ->
        Diagnostics.report ~start_pos ~end_pos:(make_position t)
          (Diagnostics.UnknownCharacter '|') )
  | `Chr '!' -> (
    match peek t with
    | `Chr '=' ->
        next_n ~n:2 t ; Token.NOT_EQUAL
    | _ ->
        next t ; Token.NOT )
  | `Chr '=' -> (
    match peek t with
    | `Chr '=' ->
        next_n ~n:2 t ; Token.EQUAL_EQUAL
    | _ ->
        next t ; Token.EQUAL )
  | `Chr '>' -> (
    match peek t with
    | `Chr '=' ->
        next_n ~n:2 t ; Token.GREATER_EQUAL
    | _ ->
        next t ; Token.GREATER )
  | `Chr '*' -> (
    match peek t with
    | `Chr '*' ->
        next_n ~n:2 t ; Token.STAR_STAR
    | _ ->
        next t ; Token.STAR )
  | `Chr '<' -> (
    match peek t with
    | `Chr '>' ->
        next_n ~n:2 t ; Token.HTML_OPEN_FRAGMENT
    | `Chr '-' ->
        next_n ~n:2 t ; Token.ARROW_LEFT
    | `Chr '=' ->
        next_n ~n:2 t ; Token.LESS_EQUAL
    | `Chr 'a' .. 'z' ->
        setMode Template t ;
        setMode TemplateAttributes t ;
        scan_open_tag t
...
and scan_token ~start_pos t =
  ...
  | Template ->
      scan_template_token ~start_pos t
  | TemplateAttributes ->
      scan_template_attributes_token ~start_pos t
  | ComponentAttributes ->
      scan_component_attributes_token ~start_pos t
  | String ->
      scan_string_token ~start_pos t
  | Normal ->
      scan_normal_token ~start_pos t
