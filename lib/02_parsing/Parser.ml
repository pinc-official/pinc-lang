...
module Rules = struct
  let rec parse_string_template t =
    ...
    let* string_template_desc =
      ...
      | Token.OPEN_TEMPLATE_LITERAL ->
          ...
          Some (Parsetree.P_StringInterpolation (P_Lowercase_Id identifier))
      | _ -> None
    in
    ...
    in
    Parsetree.{ string_template_desc; string_template_loc } |> Option.some
  ...
  and parse_tag ~name t =
    ...
    let tag =
      match name with
      | "String" -> Parsetree.P_Tag_String
      | "Int" -> Parsetree.P_Tag_Int
      | "Float" -> Parsetree.P_Tag_Float
      | "Boolean" -> Parsetree.P_Tag_Boolean
      | "Array" -> Parsetree.P_Tag_Array
      | "Record" -> Parsetree.P_Tag_Record
      | "Slot" -> Parsetree.P_Tag_Slot
      | "Store" -> Parsetree.P_Tag_Store
      | "SetContext" -> Parsetree.P_Tag_SetContext
      | "GetContext" -> Parsetree.P_Tag_GetContext
      | "CreatePortal" -> Parsetree.P_Tag_CreatePortal
      | "Portal" -> Parsetree.P_Tag_Portal
      | other -> Parsetree.P_Tag_Custom other
    in
    let tag_loc = Location.merge ~s:start_token.location ~e:t.token.location () in
    let tag_desc = Parsetree.{ tag; attributes; transformer } in
  ...
  and parse_template_node t =
    ...
    let* template_node_desc =
      ...
      | Token.COMPONENT_OPEN_TAG identifier ->
          ...
          in
          Some
            (Parsetree.P_ComponentTemplateNode
               {
  ...
  and parse_expression_part t =
    ...
    let* expression_desc =
      ...
      | Token.COMMENT s ->
          next t;
          Some (Parsetree.P_Comment s)
      (* PARSING PARENTHESIZED EXPRESSION *)
      | Token.LEFT_PAREN ->
          next t;
          let expr = parse_expression t in
          t |> expect Token.RIGHT_PAREN;
          expr |> Option.map (fun expr -> expr.Parsetree.expression_desc)
      ...
      | Token.LEFT_BRACE ->
          ...
          if is_record then (
            ...
            in
            t |> expect Token.RIGHT_BRACE;
            Parsetree.P_Record attrs |> Option.some)
          else
            let* expr = parse_block t in
            Some expr.Parsetree.expression_desc
      ...
      | Token.KEYWORD_FOR -> (
          ...
          let index, iterator =
            if optional Token.COMMA t then (
              let identifier = Helpers.expect_identifier ~typ:`Lower t in
              (Some (Parsetree.P_Lowercase_Id index_or_iterator), identifier))
            else
              (None, index_or_iterator)
          in
          let iterator = Parsetree.P_Lowercase_Id iterator in
      ...
      | Token.KEYWORD_FN -> (
          ...
          | Token.EXTERNAL_FUNCTION_SYMBOL name ->
              next t;
              Option.some @@ Parsetree.P_ExternalFunction { parameters; name }
          | _ ->
              let* body = t |> parse_block in
              Option.some @@ Parsetree.P_Function { parameters; body })
      ...
      | Token.HTML_DOCTYPE s ->
          ...
          in
          Parsetree.P_TemplateExpression [ template_nodes ] |> Option.some
      ...
      | Token.IDENT_LOWER identifier ->
          next t;
          Parsetree.P_LowercaseIdentifierExpression identifier |> Option.some
      | Token.IDENT_UPPER identifier -> (
          ...
          | [ identifier ] ->
              Parsetree.P_UppercaseIdentifierExpression identifier |> Option.some
          | path -> Parsetree.P_UppercaseIdentifierPathExpression path |> Option.some)
      (* PARSING VALUE EXPRESSION *)
      | Token.DOUBLE_QUOTE ->
          next t;
          let s = t |> Helpers.list ~fn:parse_string_template in
          t |> expect Token.DOUBLE_QUOTE;
          Parsetree.(P_String s) |> Option.some
      | Token.INT i ->
          next t;
          Parsetree.(P_Int i) |> Option.some
      | Token.CHAR c ->
          next t;
          Parsetree.(P_Char c) |> Option.some
      | Token.FLOAT f ->
          next t;
          Parsetree.(P_Float f) |> Option.some
      | Token.KEYWORD_TRUE ->
          next t;
          Parsetree.(P_Bool true) |> Option.some
      | Token.KEYWORD_FALSE ->
          next t;
          Parsetree.(P_Bool false) |> Option.some
      | Token.LEFT_BRACK ->
          next t;
          let expressions =
          ...
          Parsetree.(P_Array expressions) |> Option.some
