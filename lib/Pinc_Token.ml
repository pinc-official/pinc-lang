...
let to_string = function
  ...
  | HTML_CLOSE_TAG s ->
      "</" ^ s
  | COMPONENT_OPEN_TAG s ->
      "<" ^ s
  | COMPONENT_CLOSE_TAG s ->
      "</" ^ s
  | HTML_OR_COMPONENT_TAG_SELF_CLOSING ->
      "/>"
  | HTML_OR_COMPONENT_TAG_END ->
...
let keyword_of_string = function
  ...
  | "in" ->
      Some KEYWORD_IN
  | "reverse" ->
      Some KEYWORD_REVERSE
  | "break" ->
      Some KEYWORD_BREAK
  | "continue" ->
      Some KEYWORD_CONTINUE
  | "component" ->
      Some KEYWORD_COMPONENT
  | "library" ->
      Some KEYWORD_LIBRARY
  | "site" ->
      Some KEYWORD_SITE
  | "page" ->
      Some KEYWORD_PAGE
...
let lookup_keyword str =
  ...
  | Some t ->
      t
  | None -> (
    match str.[0] with 'A' .. 'Z' -> IDENT_UPPER str | _ -> IDENT_LOWER str )
