...
let get_files_with_ext ~ext dir =
  let rec loop result = function
    | file :: rest when Sys.is_directory file ->
        ...
        |> List.append rest |> loop result
    | file :: rest when Filename.extension file = ext ->
       ...
       (fun acc filename ->
         ...
         let f key x y =
           ...
           | Some _, Some _ ->
               failwith
                 (Printf.sprintf
                    "Found multiple declarations with identifier %s" key )
           | None, None ->
               None
         in
...
let main =
  ...
  declarations |> Interpreter.eval ~root |> Interpreter.State.get_output
  |> Interpreter.Value.to_string |> print_endline
